(function () {
    ...
    var listItemEl = function (item, position) {
        return '<li class="item">\n' + '<div class="item-date">\n' + item.date + '</div>' + '<div class="item-title">\n' + item.title + '</div>' + '<div class="item-description">' + item.desc + '</div>' + '<iframe src="' + item.url + '" scrolling="no" border="no" maginwidth="0" marginheight="0" frameborder="0" ></iframe>' + '<div class="link"><a href="' + item.url + '">보러 가기</a></div>' + (position !== items.length - 1 ? '<div style="width: 100px;margin-left: auto;margin-right:auto;margin-top:2rem;"><hr></div>' : '') + '</li>';
    };
    ...
    OPUS.select('.music-player-control a').execute(function (o) {}).on('click', function (e) {
